...
uint64_t DASB_ECBS::timeSinceEpochMillisec()
{
  using namespace std::chrono;
  return duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
}
DASB_ECBS::DASB_ECBS(Problem *_P, float _w) : CBS(_P, false), w(_w)
{
  init();
}
DASB_ECBS::DASB_ECBS(Problem *_P, float _w, bool _ID) : CBS(_P, _ID), w(_w)
{
  init();
}
...
void DASB_ECBS::invoke(CTNode *node, Agents &block)
{
  int d;
  // calc path
  if (node->c.empty())
...
int DASB_ECBS::h3(Agent *a, Nodes &p1, Paths &paths)
{
  if (p1.empty())
    return 0;
...
Nodes DASB_ECBS::AstarSearch(Agent *a, CTNode *node)
{
...
void DASB_ECBS::getPartialPath(AN *n, Nodes &path)
{
  path.clear();
  AN *m = n;
  while (m != nullptr)
