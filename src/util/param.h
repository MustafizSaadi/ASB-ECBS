               ...
               Param::VisualConfig* visual)
{
  ...
  std::regex r_timelimit = std::regex(R"(timelimit=(\d+))");
  ...
  while (getline(file, line)) {
    if (std::regex_match(line, results, r_problem_type)) {
      ...
      } else if(tmpstr == "SASB_ECBS"){
        env->STYPE = Param::SOLVER_TYPE::S_SASB_ECBS;
      } else if(tmpstr == "DASB_ECBS"){
        env->STYPE = Param::SOLVER_TYPE::S_DASB_ECBS;
    ...
    } else if (std::regex_match(line, results, r_timelimit)) {
      env->timelimit = std::stoi(results[1].str());
